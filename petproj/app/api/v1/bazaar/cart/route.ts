...
export async function GET(req: NextRequest) {
    try {
        ...
        if (cartRes.rowCount === 0) return NextResponse.json({ items: [], total_price: 0 });
        ...
        const items = await db.query(`
            SELECT 
                ...
                (SELECT image_url FROM bazaar_product_media WHERE product_id = p.product_id LIMIT 1) as image_url
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (existing.rowCount > 0) {
            ...
            await db.query(`
                INSERT INTO bazaar_cart_items (cart_id, product_id, variant_id, quantity)
                VALUES ($1, $2, $3, $4)
