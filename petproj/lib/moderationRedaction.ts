 ...
 *
 * content_notice_reason (posts only — see lib/moderation/petSaleDetection.ts
 * and the admin moderate route) is a third, unrelated case: it's public on
 * purpose. A flagged post stays fully visible to every viewer, same as a
 * normal post; the reason is just metadata the client reads to render a
 * small "Paltuu doesn't condone this" badge. So unlike is_shadow_hidden,
 * this function deliberately leaves it untouched on every row.
 *
