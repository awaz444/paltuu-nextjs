...
export async function POST(req: NextRequest) {
    try {
        ...
        // lib/moderation/petSaleDetection.ts) does NOT hide the post — Paltuu
        // doesn't condone selling pets, but the post stays fully public like
        // any other, fans out normally, etc. content_notice_reason just tags
        // it so the client can render a small "Paltuu doesn't condone this"
        // badge to every viewer. Independent of autoRedact/moderation_state —
        // a post can be both redacted (a slur covered) and pet-sale-flagged.
        ...
        try {
            ...
            const postRes = await client.query(`
                INSERT INTO social_posts (user_id, post_type, content, moderation_state, is_shadow_hidden, content_notice_reason)
                VALUES ($1, $2, $3, $4, $5, $6)
                RETURNING *
            `, [userId, post_type, content, autoRedact ? 'redacted' : 'none', false, autoPetSale ? 'pet_sale' : null]);
            ...
            // A pet-sale flag doesn't affect visibility, so this always runs
            // regardless of autoPetSale.
