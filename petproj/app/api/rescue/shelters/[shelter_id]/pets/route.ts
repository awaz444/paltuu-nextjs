/**
 * @swagger
 * /api/rescue/shelters/[shelter_id]/pets:
 *   get:
 *     summary: Auto-generated summary for /api/rescue/shelters/[shelter_id]/pets
 *     tags: [Auto-Generated]
 */
...
export async function GET(req: NextRequest, { params }: { params: { shelter_id: string } }): Promise<NextResponse> {
    const shelter_id = params.shelter_id;  // Extracting shelter_id from the dynamic URL

    if (!shelter_id || isNaN(Number(shelter_id))) {
        return NextResponse.json(
            { error: "Bad Request", message: "Invalid or missing shelter_id" },
            { status: 400 }
        );
    }
    ...
    try {
        ...
        if (result.rows.length === 0) {
            return NextResponse.json(
                { error: "No pets found for this shelter" },
                { status: 404 }
            );
        }
