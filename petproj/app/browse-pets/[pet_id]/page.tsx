  ...
  useEffect(() => {
    const fetchPetDetails = async () => {
      try {
        ...
        if (petData.profile_image_url && petData.profile_image_url !== petData.owner_profile_image) {
          images.push(petData.profile_image_url);
        }
        
        if (petData.additional_images && petData.additional_images.length > 0) {
          petData.additional_images.forEach(
            (image: { image_url: string }) => {
              if (image.image_url && image.image_url !== petData.owner_profile_image) {
          ...
          <Card className="shadow-xl rounded-2xl overflow-hidden border-none">
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="lg:w-1/2 space-y-6">
                ...
                {/* Adoption Status Tag */}
                {!isAvailable && (
                  <Tag color="red" className="rounded-full px-4 py-1 text-base">
                    Already Adopted
                  </Tag>
                )}
                ...
                <Row gutter={16}>
                  ...
                  {hasValue(pet.sex) && (
                    <Col span={hasValue(pet.price) ? 12 : 24}>
                      <div className="bg-gray-50 p-4 rounded-xl h-full">
                        <Text className="text-sm font-semibold text-gray-500 block mb-1">
                          Sex
                        </Text>
                        <Title level={4} className="text-gray-800 m-0 capitalize">
                          {pet.sex}
                        </Title>
                      </div>
                    </Col>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <Button
                    size="large"
                    className="flex-1 h-14 text-lg font-semibold rounded-xl border-primary text-primary"
            ...
            <div className="space-y-6">
              {hasValue(pet.description) && (
                <div className="p-5 rounded-lg border border-gray-200">
                  <Title level={3} className="text-gray-800 mb-3 flex items-center">
                    <InfoCircleOutlined className="mr-2" /> About {pet.pet_name}
                  </Title>
              ...
              {(hasValue(pet.energy_level) || hasValue(pet.cuddliness_level)) && (
                <div className="p-5 rounded-lg border border-gray-200">
                  <Title level={3} className="text-gray-800 mb-4 flex items-center">
                    Personality Traits
                  </Title>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {(hasValue(pet.vaccinated) || hasValue(pet.neutered) || hasValue(pet.health_issues)) && (
                  <div className="p-5 rounded-lg border border-gray-200">
                    <Title level={3} className="text-gray-800 mb-3 flex items-center">
                      <SafetyCertificateOutlined className="mr-2" /> Health & Care
                    </Title>
                    <div className="space-y-2">
                      ...
                      {hasValue(pet.health_issues) && (
                        <div className="p-2 bg-gray-50 rounded">
                          <span className="font-medium block mb-1">Health Notes</span>
                          <span className="text-gray-700">{pet.health_issues}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {(hasValue(pet.can_live_with_dogs) || hasValue(pet.can_live_with_cats) || hasValue(pet.must_have_someone_home)) && (
                  <div className="p-5 rounded-lg border border-gray-200">
                    <Title level={3} className="text-gray-800 mb-3 flex items-center">
                      <HomeOutlined className="mr-2" /> Living Preferences
                    </Title>
                    <div className="space-y-2">
                      {hasValue(pet.can_live_with_dogs) && (
                        <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                          <span className="font-medium">Good with Dogs</span>
                          <span className="font-semibold">
                      ...
                      {hasValue(pet.can_live_with_cats) && (
                        <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                          <span className="font-medium">Good with Cats</span>
                          <span className="font-semibold">
                      ...
                      {hasValue(pet.must_have_someone_home) && (
                        <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                          <span className="font-medium">Needs Company</span>
                          <span className="font-semibold">
