 ...
 * Returns posts tagged with a content tag (e.g. "cute", "training"),
 * newest first — powers the Explore page's curated topic rails.
 */
export async function GET(req: NextRequest, { params }: { params: { slug: string } }) {
    try {
        ...
        const tagRes = await db.query(
            "SELECT tag_id, slug, label FROM content_tags WHERE slug = $1 AND is_active = true",
            [slug]
        );

        if (tagRes.rowCount === 0) {
            return errorResponse("NOT_FOUND", "Topic not found", 404);
        }

        const tag = tagRes.rows[0];
        const queryParams: any[] = [tag.tag_id, userId, limit + 1];
        ...
        let query = `
            ...
            JOIN post_content_tags pct ON pct.post_id = p.post_id
        ...
        console.error("V1 Topic Feed error:", error);
