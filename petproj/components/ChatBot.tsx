...
export default function ChatBot() {
    ...
    const hideChatbot =
        ...
        pathname.startsWith("/rescue-register");
    ...
    return (
        <div
            className={`fixed right-0 z-49 transition-all duration-300 ${
                hasListingButton
                    ? isOpen
                        ? "bottom-4 right-4"
                        : "bottom-4 right-4"
            ...
            {!isOpen && (
                <button
                    ...
                    className="relative bg-white text-primary py-2 px-2 rounded-xl shadow-sm overflow-hidden flex flex-col items-center justify-center border-2 border-transparent border-primary hover:scale-102 transition-all duration-300 text-sm sm:text-base sm:p-2 sm:rounded-xl">
                    <div className="flex flex-row items-center gap-1 sm:gap-2">
                        <MessageSquare size={12} className="sm:w-4 sm:h-4" />
                        <span className="text-xs sm:text-sm">Paltuu AI</span>
                    </div>
                    {hasNewMessage && (
                        <span className="absolute top-2 right-2 w-2 h-2 sm:w-3 sm:h-3 bg-red-500 rounded-full animate-pulse" />
                    )}
                </button>
            )}
