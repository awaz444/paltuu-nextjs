...
interface PetCareClientProps {
    initialClinics: Clinic[];
    initialPagination: { page: number; limit: number; total: number; totalPages: number; hasMore: boolean };
    initialCities: string[];
}

export default function PetCareClient({ initialClinics, initialPagination, initialCities }: PetCareClientProps) {
    const dispatch = useDispatch<AppDispatch>();
    const {
        clinics: reduxClinics,
        cities: reduxCities,
        ...
        mapClinics,
        mapLoading,
    } = useSelector((state: RootState) => state.clinics);
    // The very first render (SSR + initial client hydration pass) shows the
    // server-fetched data directly so crawlers and first paint see real
    // clinic content, not an empty shell. Once the client's own fetch
    // resolves, Redux state takes over for everything after (filters,
    // geolocation-based re-sort, infinite scroll).
    const [clientFetched, setClientFetched] = useState(false);
    const clinics = clientFetched ? reduxClinics : initialClinics;
    const cities = clientFetched ? reduxCities : initialCities;
    const pagination = clientFetched ? reduxPagination : initialPagination;
