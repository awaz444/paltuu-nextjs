...
// GET method to fetch pending reviews for a specific vet_id
export async function GET(req: NextRequest, { params }: { params: { vet_id: string } }): Promise<NextResponse> {
    ...
    WHERE vr.vet_id = $1 AND vr.approved = false
...
`,
        ...
        if (result.rows.length === 0) {
            return NextResponse.json({ message: 'No pending reviews found for this vet' }, { status: 404 });
        }
