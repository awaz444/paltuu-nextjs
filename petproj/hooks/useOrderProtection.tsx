  ...
  useEffect(() => {
    const checkOrder = async () => {
      ...
      try {
        ...
        // Verify the order exists and belongs to current session
        const response = await fetch(`/api/bazaar/orders?orderNumber=${encodeURIComponent(orderNumber)}&sessionId=${encodeURIComponent(sessionId)}`);
        ...
        const cartResponse = await fetch(`/api/bazaar/cart?sessionId=${encodeURIComponent(sessionId)}`);
