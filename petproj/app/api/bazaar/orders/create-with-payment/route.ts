...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const proofResult = await client.query(proofQuery, [
        order.order_id,
        userId || null,
        sessionId || null,
        paymentProofUrl,
      ...
      // 7. Clear cart items
      if (cartData.cartId) {
        await client.query('DELETE FROM bazaar_cart_items WHERE cart_id = $1', [cartData.cartId]);
      }
      ...
      const completeOrderResult = await client.query(completeOrderQuery, [order.order_id]);
