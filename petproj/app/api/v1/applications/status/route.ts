    ...
    try {
        ...
        if (status === 'approved') {
            ...
            await db.query(`
                ...
                VALUES ($1, $2, '${type}_approved', false, NOW())
            `, [application.user_id, `Congratulations! Your ${type} application for ${application.pet_name} has been approved.`]);

            for (const row of rejectResult.rows) {
                await db.query(`
                    INSERT INTO notifications (user_id, notification_content, notification_type, is_read, date_sent)
                    VALUES ($1, $2, '${type}_rejected', false, NOW())
                `, [row.user_id, `Your ${type} application for ${application.pet_name} was not selected.`]);
            }
        } else {
            ...
            await db.query(`
                INSERT INTO notifications (user_id, notification_content, notification_type, is_read, date_sent)
                VALUES ($1, $2, '${type}_rejected', false, NOW())
            `, [application.user_id, `Your ${type} application for ${application.pet_name} has been rejected.`]);
        }
