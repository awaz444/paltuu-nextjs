...
interface StepTwoProps {
  data: StepTwoData;
  setData: (updatedData: Partial<StepTwoData>) => void;
  next: () => void;
  back: () => void;
}
...
const StepTwo: React.FC<StepTwoProps> = ({
  ...
  const handleInputChange = (field: keyof Omit<StepTwoData, 'socials'>, value: string) => {
    setData({ [field]: value });
  };

  const handleSocialChange = (platform: keyof SocialsData, value: string) => {
    setData({
      socials: {
        ...data.socials,
        [platform]: value
  ...
  return (
    <div className="w-full max-w-md bg-white shadow-lg rounded-3xl p-6 space-y-4">
      <h2 className="text-2xl font-semibold text-center">Banking & Socials</h2>
      ...
      <div className="space-y-4 pt-4">
        ...
        <div>
          <label className="block text-gray-600 text-xs mb-1">Website</label>
          <input
            type="text"
