...
export default function ProfileContent() {
  ...
  const handlePersonalInfoChange = (field: keyof UserProfileData, value: string) => {
    setUpdatedData((prev) => (prev ? { ...prev, [field]: value } : null));
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      message.error({ content: "Failed to update profile", duration: 3 });
    }
  };
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        <div className="md:col-span-1 flex flex-col items-center">
          ...
          {editing && (
            <p className="text-sm text-gray-500 mt-2 text-center">Click image to update</p>
          )}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1">
              ...
              {editing ? (
                ...
                <p className="p-2 bg-gray-50 rounded-lg">+92{updatedData?.phone_number}</p>
              )}
            </div>
            <ProfileField
              label="City"
